...
if busybox cat $chip_type_path | toolbox grep RK903; then
  module_path=$module_path_rk903
  chip_broadcom=true
  echo 1 > $pcba_node
fi

if busybox cat $chip_type_path | toolbox grep RK901; then
  module_path=$module_path_rk903
  chip_broadcom=true
  echo 1 > $pcba_node
fi

if busybox cat $chip_type_path | toolbox grep BCM4330; then
  module_path=$module_path_rk903
  chip_broadcom=true
  echo 1 > $pcba_node
fi

if busybox cat $chip_type_path | toolbox grep RTL8188CU; then
  jmax=6
  module_path=$module_path_8192cu
fi

if busybox cat $chip_type_path | toolbox grep RTL8188EU; then
  jmax=6
  module_path=$module_path_8188eu
fi

if busybox cat $chip_type_path | toolbox grep RT5370; then
  jmax=6
  module_path=$module_path_rt5370
fi

if busybox cat $chip_type_path | toolbox grep MT7601; then
  jmax=6
...
if busybox cat $chip_type_path | toolbox grep RTL8723AU; then
  module_path=$module_path_8723au
fi

if busybox cat $chip_type_path | toolbox grep RTL8723AS; then
  module_path=$module_path_8723as
fi  

if busybox cat $chip_type_path | toolbox grep ESP8089; then
  module_path=$module_path_esp8089
fi

if busybox cat $version_path | toolbox grep 3.0.36+; then
...
if busybox ls /dev/wmtWifi | toolbox grep wmtWifi; then
...
do
    ...
    if busybox ifconfig wlan0; then
        ...
        iwlist wlan0 scanning last | toolbox grep SSID > $result_file
        busybox cat $result_file
        iwlist wlan0 scanning last | toolbox grep "Signal level" > $result_file2
