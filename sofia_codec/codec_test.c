...
void IntMic_LS_test()
{
	...
	int ret = system("/system/bin/audio_pcb 1 /data/test.wav -D 0 -d 0 -c 2 -r 48000 -b 16 -p 240 -n 8 -t 0 -u 5");
	if(ret < 0) {
		LOG("IntMic_LS_test:: cmd: ./audio_pcb error: %s", strerror(errno));
	}

	ret = system("/system/bin/audio_pcb 2 /data/test.wav -D 0 -d 0 -p 240 -n 16 -t 1 -u 5");
	if(ret < 0) {
...
void EP_test()
{
	...
	int ret = system("/system/bin/audio_pcb 2 /system/etc/test.wav -D 0 -d 0 -p 240 -n 16 -t 0 -u 11");
	if(ret < 0) {
